...
- **loop-cards**（`component/loop-cards.js` + 同名 `.css`，当前无页面在用）：容器 `<div class="loop-cards">`，
  ...
  - 轨道是**三段式**：前导克隆 ×N + 原始卡片 ×N + 尾部克隆 ×N（共 3N）。位移锚定在
    中间一圈的原始卡片上：`canonicalOffset(m) = -(span + cardOffsets[m])`，取值恒在
    `[-2span, -span]`，视口两侧都有克隆垫着 —— **两端各一圈克隆是「上一张 / 下一张」
    跨首尾能无缝续上的前提**（只留尾部一圈时，首卡往左滑会滑出空白）
  - ⚠️ **卡片宽度不必相等**：`cardOffsets[m]` 必须是**实测**的每卡位置（用相邻两卡间距
    当步长会逐张累积错位，三张不等宽时误差可达上百 px）
  - ⚠️ **「等价位移」要分两种用法**（周期 span 的位移画面完全相同，但用途不同）：
    · **步进（上一张/下一张）必须按行进方向过滤**（`directedStepOffset(…, sign)`）：
      出界那一步同时存在「前进一整圈的落点」与「倒退一整圈的落点」，若只比距离，
      当「末张卡占位 < 整圈一半」时反向那个更近 → 内容朝反方向滑 = **表现为滚回开头**
      （仅在特定宽度组合下触发，等宽卡片反而看不出）
    · **跳转（点进度点）取最近的**（`nearestOffset`）—— 跳转只要最短动画
  - ⚠️ **带过渡时，归位必须无过渡**：连续模式（每帧约 1.2px、无过渡）直接 `offset += span`
    看不出等价性；但停顿模式用 CSS 过渡，若在 `apply()` 前就加 span，浏览器会把
    「倒退一圈」演成整条轨道倒滑回开头。正确做法：步进只走正常一步（越界那步落在克隆上），
    过渡结束后再 `style.transition='none'` + 强制重排 + 恢复，瞬间归位
  - 进度条填充用**宽度**驱动（`width: calc(--p * 100%)` + 与底衬相同的 `border-radius`）：
    `transform: scaleX(--p)` 会把圆角一并横向压扁，右端变椭圆帽、与底衬对不上
  ...
  - 纯逻辑已导出便于自检：`secondsToMs` / `resolveArrow` / `resolveController` /
    `nearestOffset` / `directedStepOffset`
