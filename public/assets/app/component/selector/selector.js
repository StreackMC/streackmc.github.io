 ...
 *   两者互斥
 *
 * 模块形式：ESM（原 IIFE 包装已移除）；初始化统一走框架的 requestInitFunc，
 * URL 参数读取统一走 frame/utils.js 的 getQueryString。
 */

import { requestInitFunc } from '../../frame/init-hooks.js';
import { getQueryString } from '../../frame/utils.js';
 ...
 * URL 读取统一走框架的 getQueryString（frame/utils.js）
 */
function applyURLParam() {
  let raw = getQueryString('selector');
  if (!raw) return;
...
/* 扫描时机：
   · 宿主页面加载了框架（window.streack 已建立）→ 走 requestInitFunc，
     框架就绪（DOM 已缓存、事件已绑定）后再扫描；
   · 宿主页面未加载框架（纯静态页）→ 自行在 DOM 就绪后扫描，避免静默失效。 */
if (window?.streack) {
  requestInitFunc(scanAndInit);
